                    ...
                    <form action="{{ route('admin.spareparts.update', $sparepart->id) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('nama') is-invalid @enderror"
                                id="nama" name="nama" value="{{ old('nama', $sparepart->nama) }}" required>
                            @error('nama')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control @error('jumlah') is-invalid @enderror"
                                id="jumlah" name="jumlah" value="{{ old('jumlah', $sparepart->jumlah) }}" min="0" required>
                            @error('jumlah')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('satuan') is-invalid @enderror" id="satuan" name="satuan" required>
                                ...
                                <option value="BH" {{ old('satuan', $sparepart->satuan) == 'BH' ? 'selected' : '' }}>BH</option>
                                <option value="LT" {{ old('satuan', $sparepart->satuan) == 'LT' ? 'selected' : '' }}>LT</option>
                                <option value="SET" {{ old('satuan', $sparepart->satuan) == 'SET' ? 'selected' : '' }}>SET</option>
                                <option value="UN" {{ old('satuan', $sparepart->satuan) == 'UN' ? 'selected' : '' }}>UN</option>
                                <option value="BTL" {{ old('satuan', $sparepart->satuan) == 'BTL' ? 'selected' : '' }}>BTL</option>
                            </select>
                            @error('satuan')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
