...
class BillingController extends Controller
{
    ...
    public function generatePDF(Request $request, $invoiceId)
    {
        ...
        // ✅ Step 1: Pass DomPDF options as an array
        $pdf = PDF::setOptions([
            'isHtml5ParserEnabled' => true,
            'isRemoteEnabled' => true,
            'chroot' => public_path(), // allow access to /public (for fonts/images)
        ])
        ...
        // ✅ Step 2: Sanitize the invoice number for a safe filename
        $safeInvoiceNumber = str_replace(
            ['/', '\\', ':', '*', '?', '"', '<', '>', '|'],
            '-',
            $invoice->invoice_number
        );

        // ✅ Step 3: Generate a filename and download the PDF
